			...
			<div class="post-meta">
				...
				<span class="post-title small">
					...
					{elseif $_post['post_type'] == "article"}
						{__("added blog")} {__("in")} <a href="{$system['system_url']}/blogs/category/{$_post['blog']['category_id']}/{$_post['blog']['category_url']}">{__($_post['blog']['category_name'])}</a>
					...
					{elseif $_post['post_type'] == "video"}
						{__("added video")} {__("in")} {__($_post['video']['category_name'])}
				...
				{if $_post['post_type'] == "media" && $_post['media']}
					<div class="mt-2">
						{if $_post['media']['source_type'] == "photo"}
							<div class="post-media">
								<a class="post-media-image d-block position-relative" href="{$_post['media']['source_url']}" target="_blank" rel="nofollow">
									<img src="{$_post['media']['source_url']}">
									<div class="source text-truncate text-white m-2 position-absolute">{$_post['media']['source_provider']}</div>
								</a>
							</div>
						{else}
						...
						{foreach $_post['poll']['options'] as $option}
							<div class="d-flex align-items-center gap-2">
								<div class="poll-option flex-1 js_poll-vote" data-id="{$option['option_id']}" data-option-votes="{$option['votes']}">
									<input type="radio" name="poll_{$_post['poll']['poll_id']}" id="option_{$option['option_id']}" class="d-none" {if $option['checked']}checked{/if}>
				...
				{if ($_post['post_type'] == "file" || $_post['post_type'] == "combo") && $_post['file']}
					<div class="post-downloader d-flex align-items-center gap-3 rounded-3 p-3 mt-2">
						...
						<div class="info">
							...
							<a class="btn btn-main btn-sm mt-1 text-no-underline" href="{$system['system_uploads']}/{$_post['file']['source']}">{__("Download")}</a>
						</div>
					</div>
				{/if}
			{else}
