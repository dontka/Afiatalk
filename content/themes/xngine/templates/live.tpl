		...
		<div class="bg-black h-100 live-stream-wrapper position-relative">
			...
			<div class="position-absolute w-100 bottom-0 pb-3 pb-md-4 pt-5 start-0 end-0 x_live_foot">
				...
				<div class="d-flex align-items-center justify-content-center gap-3 flex-wrap">
					...
					<span type="button" class="btn btn-danger x-hidden" id="js_live-end">{__("End")}</span>
					<span type="button" class="btn btn-main x-hidden" id="js_live-start" {if $page_id} data-node-id={$page_id} data-node='page' {/if}{if $group_id} data-node-id={$group_id} data-node='group' {/if}{if $event_id} data-node-id={$event_id} data-node='event' {/if}>{__("Go Live")}</span>
