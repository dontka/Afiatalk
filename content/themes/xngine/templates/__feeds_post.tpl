{if !$standalone}<li>{/if}
	...
	{if $post['source'] == "popular"}
		<div class="mb-2 pb-1 small text-muted">{__("Popular")}</div>
	{else if $post['source'] == "discover"}
		<div class="mb-2 pb-1 small text-muted">{__("Suggested for you")}</div>
	{/if}
	...
	<div class="bg-white position-relative p-3 post 
                ...
                {if ($post['is_pending']) OR ($post['in_group'] && !$post['group_approved']) OR ($post['in_event'] && !$post['event_approved'])}pending{/if}
					...
					<div class="flex-1">
						...
						<div class="post-stats d-flex align-items-center">
							...
							{if $post['post_type'] == "video"}
								<span class="fw-bold mx-1">·</span>{$post['video']['views']} {__("Views")}
							{/if}
							{if $post['post_type'] == "shared" && $post['origin']['post_type'] == "video"}
								<span class="fw-bold mx-1">·</span>{$post['origin']['video']['views']} {__("Views")}
							{/if}
							<!-- video views -->

							<!-- audio views -->
							{if $post['post_type'] == "audio"}
								<span class="fw-bold mx-1">·</span>{$post['audio']['views']} {__("Views")}
							{/if}
							{if $post['post_type'] == "shared" && $post['origin']['post_type'] == "audio"}
								<span class="fw-bold mx-1">·</span>{$post['origin']['audio']['views']} {__("Views")}
							{/if}
							<!-- audio views -->
