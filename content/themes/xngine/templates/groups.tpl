		...
		<div class="position-sticky x_top_posts">
				...
				<span class="flex-0 d-flex align-items-center gap-2">
					...
					{if $user->_data['can_create_groups']}
						<button class="btn btn-sm btn-primary flex-0 d-none d-md-flex" data-toggle="modal" data-url="modules/add.php?type=group">
							<span class="my2">{__("Create Group")}</span>
						</button>
			...
			<div class="d-flex align-items-center justify-content-center">
				<div {if $view == "" || $view == "category"}class="active fw-semibold" {/if}>
					<a href="{$system['system_url']}/groups" class="body-color side_item_hover w-100 text-center d-block">
						<span class="position-relative d-inline-block py-3">{__("Discover")}</span>
					</a>
				</div>
				{if $user->_logged_in}
					<div {if $view == "joined"}class="active fw-semibold" {/if}>
						<a href="{$system['system_url']}/groups/joined" class="body-color side_item_hover w-100 text-center d-block">
							<span class="position-relative d-inline-block py-3">{__("Joined Groups")}</span>
						</a>
					</div>
					<div {if $view == "manage"}class="active fw-semibold" {/if}>
						<a href="{$system['system_url']}/groups/manage" class="body-color side_item_hover w-100 text-center d-block">
							<span class="position-relative d-inline-block py-3">{__("My Groups")}</span>
						</a>
					</div>
				{/if}
			</div>
		...
		<div class="pt-3 pb-2 px-2 mx-1">
			<form class="js_search-form" data-filter="groups">
				<div class="position-relative">
					<input type="search" class="form-control shadow-none rounded-pill x_search_filter" name="query" placeholder='{__("Search for groups")}'>
					...
					<ul class="px-3 d-flex gap-2 align-items-center overflow-x-auto pb-3 scrolll">
						...
						{else}
							{if $current_category['parent']}
								<li>
									<a class="btn btn-sm border-0 ps-0 pe-1" href="{$system['system_url']}/groups/category/{$current_category['parent']['category_id']}/{$current_category['parent']['category_url']}">
							...
							{else}
								<li>
									<a class="btn btn-sm border-0 ps-0 pe-1" href="{$system['system_url']}/groups">
						...
						{foreach $categories as $category}
							<li {if $view == "category" && $current_category['category_id'] == $category['category_id']}class="position-relative main main_bg_half" {/if}>
								<a class="btn btn-sm" href="{$system['system_url']}/groups/category/{$category['category_id']}/{$category['category_url']}">
