	...
	<div class="card-register bg-white rounded-3 mb-4">
		...
		<div class="js_panel {if $do == 'up'}x-hidden{/if}">
			<div class="card-header">
				<h4 class="card-title headline-font fw-bold mb-2">{__("Sign In to your Account")}</h4>
			...
			<div class="card-body pt0">
				...
				<form class="js_ajax-forms" data-url="core/signin.php" method="POST">
					...
					<div class="form-floating">
						...
						<label>{__("Email")} {__("or")} {__("Username")}</label>
					</div>
					<!-- username or email -->
					<!-- password -->
					...
					<div class="d-flex align-items-center justify-content-between gap-2 flex-wrap">
						...
						<a class="" href="{$system['system_url']}/reset">{__("Forgotten password?")}</a>
					</div>
					<!-- remember me -->
					<!-- submit -->
					<div class="mt-3">
						<button type="submit" class="btn btn-lg w-100 btn-primary">{__("Sign In")}</button>
					</div>
					<!-- submit -->
					<!-- error -->
					<div class="alert alert-danger mt15 mb0 x-hidden"></div>
				...
				{if $system['registration_enabled']}
					<div class="mt-4">
						{__("Do not have an account?")} <a href="{$system['system_url']}/signup{if $oauth_app_id}?oauth_app_id={$oauth_app_id}{/if}" class="js_toggle-panel fw-medium">{__("Sign Up")}</a>
					</div>
				{/if}
			</div>
		</div>
		...
		<div class="js_panel {if $do != 'up'}x-hidden{/if}">
			<div class="card-header">
				<h4 class="card-title headline-font fw-bold mb-3">{__("Sign Up for an Account")}</h4>
			</div>
			<div class="card-body pt0">
				<form class="js_ajax-forms" data-url="core/signup.php" method="POST">
					{if $system['invitation_enabled']}
					...
					{if $system['activation_enabled'] && $system['activation_type'] == "sms"}
						...
						<div class="form-floating">
							...
							<label>{__("Phone number (e.g: +1234567890)")}</label>
						</div>
						<!-- phone -->
					{/if}
					...
					<div class="js_hidden-section x-hidden">
						...
						{if $system['select_user_group_enabled'] && $user_groups}
							...
							<div class="form-floating">
								...
								<label>{__("Select User Group")}</label>
							</div>
							<!-- user group -->
						{/if}
					...
					<div class="mt-3">
						{if $oauth_app_id}
							<input type="hidden" name="oauth_app_id" value="{$oauth_app_id}">
						{/if}
						<button type="submit" class="btn btn-lg w-100 btn-primary">{__("Sign Up")}</button>
					</div>
					<!-- submit -->
				...
				<div class="mt-4">
					{__("Already have an account?")} <a href="{$system['system_url']}/signin" class="js_toggle-panel fw-medium">{__("Sign In")}</a>
				</div>
			</div>
		</div>
		<!-- signup -->
