<li>
	<div class="conversation clearfix d-flex align-items-start py-1 gap-1 flex-wrap position-relative w-100 {if $message['user_id'] == $user->_data['user_id']}right justify-content-end{/if}" id="{$message['message_id']}">
		...
		{/if}
		<div class="conversation-body position-relative d-flex flex-column {if $message['user_id'] == $user->_data['user_id']}align-items-end{else}align-items-start{/if}">
			<!-- message text -->
			<div class="text {if $message['user_id'] == $user->_data['user_id']}js_chat-color-me{/if}">{$message['message']}</div>
			{if $message['image']}
